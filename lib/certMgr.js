...
var rootCaDir = certDir;

function initCertDir() {
...
function initRootCaDir(rootCertDir){
    if(rootCertDir) rootCaDir = rootCertDir;
    if(!isRootCAFileExists()){
        logUtil.printLog("===========", logUtil.T_ERR);
        logUtil.printLog("rootCA.crt and rootCA.key are not exists in " + rootDir, logUtil.T_ERR);
        logUtil.printLog("===========", logUtil.T_ERR);
        throw(new Error("rootCA are not exists"));
    }
    logUtil.printLog('rootCaDir: ' + rootCaDir);
}
...
function createCert(hostname,callback){
    ...
    exec(cmd,{ cwd : rootCaDir },function(err,stdout,stderr){
        if(err){
...
function generateRootCA(){
    if(isRootCAFileExists()){
        logUtil.printLog(color.yellow("rootCA exists at " + rootCaDir));
    ...
    function startGenerating(){
        ...
        clearCerts(function(){
            ...
            var spawnSteam = spawn(cmd_genRoot,['.'],{cwd:rootCaDir,stdio: 'inherit'});

            spawnSteam.on('close', function (code) {

                if(code == 0){
                    logUtil.printLog(color.green("rootCA generated"));
                    logUtil.printLog(color.green(color.bold("please trust the rootCA.crt in " + rootCaDir)));
...
module.exports.initRootCaDir = initRootCaDir;